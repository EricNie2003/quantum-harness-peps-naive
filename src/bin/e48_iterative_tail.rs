use nqueens_peps_naive::{contract_rows_wide_scalar_iterative_with_target, known_count};
use std::env;
...
fn run() -> Result<(), String> {
    ...
    if args.next().is_some() || min_n > max_n || repeats == 0 {
        return Err(
            "usage: e48_iterative_tail MIN_N MAX_N REPEATS WARMUP TARGET_TASKS_PER_THREAD"
                .to_owned(),
        );
    }
    println!(
        "N,count,verified,median_elapsed_s,min_elapsed_s,p10_elapsed_s,p90_elapsed_s,\
         peak_rss_bytes,threads,split_depth,target_tail_tasks,tail_tasks,\
    ...
    for n in min_n..=max_n {
        for _ in 0..warmup {
            contract_rows_wide_scalar_iterative_with_target(n, false, target_tasks_per_thread)?;
        }
        ...
        let profile =
            contract_rows_wide_scalar_iterative_with_target(n, true, target_tasks_per_thread)?;
        ...
        println!(
            "{n},{},{verified},{:.9},{:.9},{:.9},{:.9},{peak_rss},{},{},{},{},\
             ...
             certified_explicit_C_wide_scalar_iterative_PEPS,{},{},{}",
