...
fn run() -> Result<(), String> {
    ...
    if args.next().is_some() || min_n > max_n {
        return Err("usage: e22_dd_selected MIN_N MAX_N".to_owned());
    }
    ...
    println!(
        "N,order,count,known_count,verified,elapsed_s,peak_rss_bytes,peak_live_nodes,peak_boundary_nodes,peak_relation_nodes,allocated_nodes,terminal_count,unique_lookups,unique_hits,apply_lookups,apply_hits,relprod_lookups,relprod_hits"
    );
