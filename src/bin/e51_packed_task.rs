use nqueens_peps_naive::{
    ...
    contract_rows_wide_scalar_packed_last_six_with_target, known_count,
};
use std::env;
...
fn run() -> Result<(), String> {
    ...
    if args.next().is_some()
        || !matches!(mode.as_str(), "aos" | "packed")
        || min_n > max_n
        || repeats == 0
    {
        return Err(
            "usage: e51_packed_task MODE[aos|packed] MIN_N MAX_N REPEATS WARMUP \
             TARGET_TASKS_PER_THREAD"
                .to_owned(),
        );
    }
    println!(
        "N,mode,count,verified,median_elapsed_s,min_elapsed_s,p10_elapsed_s,p90_elapsed_s,\
         peak_rss_bytes,threads,split_depth,target_tail_tasks,tail_tasks,task_size_bytes,\
         task_storage_bytes,recursive_nodes,recursive_accepted_entries,total_accepted_entries,\
         seed_s,tail_s,profile_replay_s,repeats,warmup,algorithm_class,peak_sparse_support,\
         local_tensor_entries_examined,local_tensor_entries_accepted"
    );
    for n in min_n..=max_n {
        let solve = |profile_replay| match mode.as_str() {
            "aos" => contract_rows_wide_scalar_last_k_with_target(
                n,
                profile_replay,
                target_tasks_per_thread,
                6,
            ),
            "packed" => contract_rows_wide_scalar_packed_last_six_with_target(
                n,
                profile_replay,
                target_tasks_per_thread,
            ),
        ...
        let task_size_bytes = if mode == "packed" { 8 } else { 32 };
        let task_storage_bytes = profile.tail_tasks.saturating_mul(task_size_bytes);
        println!(
            "{n},{mode},{},{verified},{:.9},{:.9},{:.9},{:.9},{peak_rss},{},{},{},{},\
             {task_size_bytes},{task_storage_bytes},{},{},{},{:.9},{:.9},{:.9},\
             {repeats},{warmup},certified_explicit_C_wide_scalar_last6_{mode}_PEPS,\
             {},{},{}",
