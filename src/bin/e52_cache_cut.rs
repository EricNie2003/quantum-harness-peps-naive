use nqueens_peps_naive::{
    contract_rows_wide_scalar_cache_budget_last_six, contract_rows_wide_scalar_last_k_with_target,
    known_count,
};
use std::env;
...
fn run() -> Result<(), String> {
    ...
    let parameter = parse(args.next(), "TARGET_OR_CACHE_KIB")?;
    ...
    {
        return Err(
            "usage: e52_cache_cut MODE[control|cache] TARGET_OR_CACHE_KIB MIN_N MAX_N \
             REPEATS WARMUP PROFILE_REPLAY"
                .to_owned(),
        );
    }
    println!(
        "N,mode,parameter,count,verified,median_elapsed_s,min_elapsed_s,p10_elapsed_s,\
         p90_elapsed_s,peak_rss_bytes,threads,split_depth,target_tail_tasks,tail_tasks,\
         task_size_bytes,task_storage_bytes,task_bytes_per_worker,recursive_nodes,\
    ...
    for n in min_n..=max_n {
        let solve = |profile| match mode.as_str() {
            "control" => contract_rows_wide_scalar_last_k_with_target(n, profile, parameter, 6),
            "cache" => contract_rows_wide_scalar_cache_budget_last_six(n, profile, parameter),
            _ => unreachable!(),
        };
        for _ in 0..warmup {
            solve(false)?;
        ...
        let profile = solve(profile_replay)?;
        ...
        let task_storage_bytes = profile
            .tail_tasks
            .saturating_mul(std::mem::size_of::<[u64; 4]>());
        let task_bytes_per_worker =
            task_storage_bytes.div_ceil(rayon::current_num_threads().max(1));
        println!(
            "{n},{mode},{parameter},{},{verified},{:.9},{:.9},{:.9},{:.9},{peak_rss},\
             {},{},{},{},32,{task_storage_bytes},{task_bytes_per_worker},{},{},{},{:.9},\
             {:.9},{:.9},{repeats},{warmup},{profile_replay},\
             certified_explicit_C_wide_scalar_last6_cache_cut_PEPS,{},{},{}",
