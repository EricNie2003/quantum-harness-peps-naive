use nqueens_peps_naive::{contract_rows_d4_compact_parallel_generation, known_count};
use std::env;
...
fn run() -> Result<(), String> {
    ...
    if args.next().is_some() || min_n > max_n || repeats == 0 {
        return Err("usage: e31_parallel_generation SHARDS MIN_N MAX_N REPEATS".to_owned());
    }
    println!(
        "shards,N,count,verified,median_elapsed_s,min_elapsed_s,peak_rss_bytes,peak_support,\
         generation_s,sort_s,reduce_s,peak_thread_local_bytes,peak_worker_partials,\
    ...
    for n in min_n..=max_n {
        ...
        for _ in 0..repeats {
            let result = contract_rows_d4_compact_parallel_generation(n, shards)?;
