use nqueens_peps_naive::path_search::{PathKind, contract_with_path};
...
fn parse_path(value: &str) -> Result<PathKind, String> {
    match value {
        ...
        "column" => Ok(PathKind::ColumnBlocks),
        "balanced" => Ok(PathKind::BalancedRectangles),
        "greedy" => Ok(PathKind::SupportAwareGreedy),
        _ => Err("PATH must be row, column, balanced, or greedy".to_owned()),
    }
}
...
fn run() -> Result<(), String> {
    ...
    let path = parse_path(
        &args
            .next()
            .ok_or_else(|| "usage: e13_path_search PATH MIN_N MAX_N".to_owned())?,
    )?;
    let min_n = parse_usize(args.next(), "MIN_N")?;
    let max_n = parse_usize(args.next(), "MAX_N")?;
    if args.next().is_some() || min_n > max_n {
        return Err("usage: e13_path_search PATH MIN_N MAX_N".to_owned());
    }

    println!(
        "path,N,count,known_count,verified,elapsed_s,peak_rss_bytes,peak_support,peak_rank,local_tensor_entries_examined,local_tensor_entries_accepted,cartesian_pair_upper_bound,matching_entry_pairs,contractions"
    );
    for n in min_n..=max_n {
        let start = Instant::now();
        let metrics = contract_with_path(n, path)?;
