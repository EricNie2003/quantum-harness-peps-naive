use nqueens_peps_naive::{contract_rows_d4_compact_u64_radix_promoting, known_count};
use std::env;
...
fn run() -> Result<(), String> {
    ...
    if args.next().is_some() || min_n > max_n || repeats == 0 {
        return Err("usage: e33_compact_radix SHARDS MIN_N MAX_N REPEATS".to_owned());
    }
    println!(
        "shards,N,count,verified,median_elapsed_s,min_elapsed_s,peak_rss_bytes,peak_support,\
         used_u64_fast_path,generation_s,sort_s,reduce_s,peak_thread_local_bytes,\
         row_operator_candidates,row_operator_matched,entry_bytes,repeats"
    );
    for n in min_n..=max_n {
        ...
        for _ in 0..repeats {
            let result = contract_rows_d4_compact_u64_radix_promoting(n, shards)?;
