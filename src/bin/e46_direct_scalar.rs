use nqueens_peps_naive::{contract_rows_wide_scalar_with_target, known_count};
use std::env;
...
fn run() -> Result<(), String> {
    ...
    if args.next().is_some() || min_n > max_n || repeats == 0 {
        return Err("usage: e46_direct_scalar MIN_N MAX_N REPEATS WARMUP \
             TARGET_TASKS_PER_THREAD PROFILE_REPLAY"
            .to_owned());
    }
    println!(
        "N,count,verified,median_elapsed_s,min_elapsed_s,p10_elapsed_s,p90_elapsed_s,\
         peak_rss_bytes,threads,split_depth,target_tail_tasks,tail_tasks,factorial_bound,\
         used_scalar_u64,promotion_reason,residues,prefix_nodes,prefix_accepted_entries,\
         prefix_kept_entries,recursive_nodes,recursive_accepted_entries,\
         total_accepted_entries,seed_s,tail_s,profile_replay_s,repeats,warmup,\
         algorithm_class,peak_sparse_support,local_tensor_entries_examined,\
         local_tensor_entries_accepted,metrics_collected"
    );
    for n in min_n..=max_n {
        for _ in 0..warmup {
            contract_rows_wide_scalar_with_target(n, false, target_tasks_per_thread)?;
        }
        let mut elapsed = Vec::<Duration>::with_capacity(repeats);
        let mut peak_rss = 0_u64;
        let mut last_result = None;
        for _ in 0..repeats {
            let result = contract_rows_wide_scalar_with_target(n, false, target_tasks_per_thread)?;
        ...
        let profile = if profile_replay {
            contract_rows_wide_scalar_with_target(n, true, target_tasks_per_thread)?
        } else {
        ...
        let residues = profile
            ...
            .collect::<Vec<_>>()
            .join("|");
        let promotion_reason = profile.promotion_reason.as_deref().unwrap_or("none");
        println!(
            "{n},{},{verified},{:.9},{:.9},{:.9},{:.9},{peak_rss},{},{},{},{},{},\
             {},{},{},{},{},{},{},{},{},{:.9},{:.9},{:.9},{repeats},{warmup},\
             certified_explicit_C_wide_direct_scalar_PEPS,{},{},{},{}",
